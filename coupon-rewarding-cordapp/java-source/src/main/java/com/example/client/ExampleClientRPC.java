...
public class ExampleClientRPC {
    ...
    private static void logState(StateAndRef<CouponState> state) {
    ...
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        final DataFeed<Vault.Page<CouponState>, Vault.Update<CouponState>> dataFeed = proxy.vaultTrack(CouponState.class);
        final Vault.Page<CouponState> snapshot = dataFeed.getSnapshot();
        final Observable<Vault.Update<CouponState>> updates = dataFeed.getUpdates();
